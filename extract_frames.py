...
def write_manifest(source_root: Path, destination_root: Path, video_count: int, image_count: int, manifest_path: Path) -> None:
    manifest = {
...
def main() -> None:
    ...
    parser.add_argument("--manifest", type=Path, default=Path("dataset_manifest.json"), help="Optional JSON manifest used to skip repeated extraction.")
    parser.add_argument(
        "--every-nth-frame", type=int, default=1,
        help="Save one frame every N video frames (default: 1).",
    )
    parser.add_argument("--force", action="store_true", help="Force extraction even if the manifest already exists.")
    ...
    if manifest and not args.force and manifest.get("source") == str(source_root) and manifest.get("output") == str(destination_root):
        print(f"Frames already extracted for {source_root}. Skipping extraction. Manifest: {manifest_path}")
        return
    ...
    write_manifest(source_root, destination_root, len(video_files), len(image_files), manifest_path)
